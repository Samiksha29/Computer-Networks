...
int main(int argc, char *argv[ ])
{
  ...
  buff=(char *)malloc(sizeof(char)*10);
	...
	ip_add=get_ip(argv[1],"ip_address");//from name it give ip address
	if(strcmp(ip_add,"NULL")==0) //if Ip address is not found in file
	  {
	     printf("Ip address not found for that user !!!");
	     printf("\nFile tranfer fails!\n");
	     return -1;
	  }
	else if(strcmp(ip_add,"192.168.1.202")==0) //Check with IP address of self
	  {
	...
	if (fp!=-1) //check wheather file name is exist or not
		  { 
			...
			recvfrom(sockfd,ack,strlen(ack),0,(struct sockaddr *)&dest_addr,&addr_len);
		        while(ack[0]!='1') //if packet is lost
			  {
